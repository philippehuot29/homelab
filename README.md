...
- [x] Install WireGuard (migrated to LXC 103 on Proxmox, tunneled via DO VPS)
...
**Skills developed:** VM management, Linux bridge networking, firewall rule logic, NAT, VLAN theory, DMZ architecture

**Security+ relevance:** Stateful vs stateless firewalls, DMZ, network segmentation, IDS/IPS integration

- [x] Deploy OPNsense 25.1 VM via Proxmox web UI
- [x] Configure Linux bridge (br0) replacing macvtap
- [x] Deploy OPNsense 25.1 VM (2GB RAM, 32GB disk, SATA/q35)
- [x] Update WireGuard PostUp/PostDown rules for br0
- [x] OPNsense accessible at 10.10.0.1 (LAN side only — requires direct cable to nic1)
- [x] Install USB NIC and managed switch (TL-SG608E)
- [x] Build allow/deny firewall rule sets (port, protocol, IP range)
- [x] Configure NAT rules
- [x] Explore VLAN concepts and inter-VLAN routing
- [x] Document IDS/IPS integration options
- [x] Deploy FortiGate VM (interview prep) — practiced CLI triage commands; decommissioned after use
...
- [x] Deploy Nagios Core — monitor all Proxmox hosts, LXCs, and VMs
- [x] Configure check_command definitions for Pi-hole, WireGuard, OPNsense
- [x] Add HTTP service checks per container/VM
- [x] Deploy Uptime Kuma — visual availability dashboard and status page
- [x] Configure Telegram alerting for host-down and service-down events (Nagios & Uptime Kuma)

#### Layer 2 — Metrics (how is it performing?)
- [x] Deploy Prometheus in Docker
- [x] Add Node Exporter for system metrics (CPU, RAM, disk)
...
- [x] Write SPL queries: event volume baseline, error surfacing, DNS query trends
- [x] Build Splunk NOC overview dashboard (3 panels minimum)
- [x] Configure Splunk threshold alert with Telegram webhook
- [x] Deploy Loki + Alloy — lightweight log pipeline feeding into Grafana (Promtail is discontinued)
- [x] Connect Loki to Grafana as second data source
- [x] Build unified Grafana dashboard pulling Prometheus metrics + Loki logs side by side
...
- [x] Split tunnel confirmed working: 10.2.0.0/24 + 192.168.8.0/24 via WireGuard, DNS via Pi-hole
...
| Service | Allocated | Idle RAM Used | Notes |
|---------|-----------|---------------|-------|
| Proxmox VE host overhead | — | ~500MB | Host total: 5.4GB across all services |
| Pi-hole LXC (CT 101) | 512MB | ~70MB | Healthy |
| WireGuard LXC (CT 103) | 256MB | ~23MB | Minimal footprint |
| OPNsense VM (VM 100) | 2048MB | ~2048MB | VM — free -m unavailable |
| **Monitoring LXC (CT 104)** | **4096MB** | **~1044MB used** | Swap: 1024MB — monitor |
| — Splunk | 1024MB cap | ~1008MB | Capped at 1GB — crashed once at higher limit |
| — cAdvisor | — | ~514MB | Docker/LXC container metrics — no mem limit set, review after Stage 10 |
| — Grafana | — | ~117MB | Port 3000 |
| — Uptime Kuma | — | ~109MB | Port 3001 |
| — Prometheus | — | ~102MB | Port 9090 |
| — Loki | — | ~74MB | Port 3100 |
| — Promtail | — | ~36MB | Ships Proxmox syslog → Loki |
| — Nagios | — | ~25MB | Port 8080 |
| Ripper LXC (CT 106) | 512MB | ~11MB | Idle when not ripping |
| Navidrome LXC (CT 107) | 256MB | ~65MB | Healthy |
| **Current stack total** | **~7.4GB allocated** | **~3.7GB used** | |
| Suricata (host) | — | ~400MB | ⬜ Stage 7 — not yet deployed |
| Wazuh + ELK Stack | — | ~2048MB | ⬜ Stage 8 — RAM intensive |
| Authentik | — | ~750MB | ⬜ Stage 9 |
| Nextcloud stack | — | ~700MB | ⬜ Stage 10 |
| Nginx | — | ~75MB | ⬜ Stage 11 |
| Gitea | — | ~200MB | ⬜ Stage 15 |
| FreeSWITCH + FastAPI | — | ~500MB | ⬜ Stage 18 |
| K3s (lightweight Kubernetes) | — | ~500MB | ⬜ Stage 14 |
| **Full stack estimate** | — | **~11-13GB** | Wazuh + ELK is the heavy hitter |

> **Conclusion:** 16GB handles the current stack comfortably. 32GB becomes relevant when running Wazuh + ELK + multiple VMs simultaneously. cAdvisor memory limit deferred — review after Stage 10.
...
- **Security:** CompTIA Security+ hands-on complement (target: July 2026)
...
*Built and maintained by Philippe | Network & Security Professional | CompTIA Network+ ✅ | AWS Cloud Practitioner ✅ | Security+ (target July 2026) 🔄*
